...
const UnitBathroomUpdateAfter = (props) => {
  ...
  const [balconyLevelAfter, setBalconyLevelAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].after?.[0].bathroom?.[0].level,
  ]);
  const [balconyAngleAfter, setBalconyAngleAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].after?.[0].bathroom?.[0].angle,
  ]);
  const [balconyConcrateAfter, setBalconyConcrateAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].after?.[0].bathroom?.[0].concrate,
  ]);
  const [balconyPatchAfter, setBalconyPatchAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].after?.[0].bathroom?.[0].patch,
  ]);
  const [balconySubStraightAfter, setBalconySubStraightAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].after?.[0].bathroom?.[0].subStraight,
  ]);
  ...
  const handleUpdate = () => {
    axios
      ...
      .then((response) => {
        ...
        xhr.open(
          "PUT",
          `${serverClient}/api/v1/units/${props.route.params.id}/bathroom-after`
        );
        xhr.send(fmData);
