...
const KitchenAdd = (props) => {
  ...
  const unitFillData = {
    ...
    photo: [
      {
        fileName: imageName,
        uri: `${serverClient}/uploads/kitchen/` + imageName,
      },
    ],
    unit: unitId,
    building: buildingId,
  ...
  const handleUpdate = () => {
    axios
      .post(`${serverClient}/api/v1/kitchen`, unitFillData)
      .then((response) => {
        const newKitchen = response.data.data;
        ...
        xhr.open(
          "PUT",
          `${serverClient}/api/v1/kitchen/${newKitchen._id}/upload`
        );
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>KITCHEN: UNIT</Text>
      </View>
      <View style={styles.view2}>
        <ScrollView style={{ flex: 1 }}>
          <FormText
            label="Enter unit's Kitchen status: "
            placeholder="Please enter Kitchen status"
            iconName="file-text"
            value={statusOf}
            onChangeText={(value) => setStatusOf(value)}
          />
          <FormText
            label="Enter unit's Kitchen reason: "
            placeholder="Please enter Kitchen reason"
            iconName="award"
            value={reason}
            onChangeText={(value) => setReason(value)}
          />
          <FormText
            label="Enter unit's Kitchen fire rate: "
            placeholder="Please enter Kitchen fire rate"
            iconName="star"
            value={fireRating}
