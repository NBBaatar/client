...
const PlasterBoardDetail = (props) => {
  const [plasterBoard, setPlasterBoard] = useState([]);
  ...
  useEffect(() => {
    axios
      .get(`${serverClient}/api/v1/plasterboard/${props.route.params.id}`)
      .then((response) => {
        setPlasterBoard(response.data.data);
      })
  ...
  const sendEmail = () => {
    ...
    axios
      .post(
        `${serverClient}/api/v1/plasterboard/${props.route.params.id}/send`,
        {
          email: email,
        }
      )
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>All Plaster board details:</Text>
      </View>
      <Animatable.View animation="bounceInUp" style={styles.view2}>
        ...
        <View style={styles.componentContent}>
          ...
          <Text style={styles.textValueStyle}> {plasterBoard.statusOf}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Reason: </Text>
          <Text style={styles.textValueStyle}> {plasterBoard.reason}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Fire rating: </Text>
          <Text style={styles.textValueStyle}> {plasterBoard.fireRating}</Text>
        </View>
        <View
          style={{
            flexDirection: "row",
        ...
        <Image
          key={plasterBoard._id}
          source={{
            uri: plasterBoard.photo?.[0].uri,
          }}
          style={styles.item}
        />
...
export default PlasterBoardDetail;
