...
const TilesBedit = (props) => {
  ...
  const unitFillData = {
    ...
    bathroom: [
      {
        ...
        photo: [
          {
            fileName: imageName,
            uri: `${serverClient}/uploads/tiles/bathroom/` + imageName,
          },
        ],
      },
    ],
    ensuite: [
      {
        statusOf: tiles?.ensuite?.[0].statusOf,
        reason: tiles?.ensuite?.[0].reason,
        fireRating: tiles?.ensuite?.[0].fireRating,
        photo: [
          {
            fileName: tiles?.ensuite?.[0].photo?.[0].fileName,
  ...
  const handleUpdate = () => {
    axios
      ...
      .then((response) => {
        ...
        xhr.open(
          "PUT",
          `${serverClient}/api/v1/tiles/${props.route.params.id}/upload/bathroom`
        );
