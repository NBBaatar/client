...
const WardrobeAdd = (props) => {
  ...
  const unitFillData = {
    ...
    photo: [
      {
        fileName: imageName,
        uri: `${serverClient}/uploads/wardrobe/` + imageName,
      },
    ],
    unit: unitId,
    building: buildingId,
  ...
  const handleUpdate = () => {
    axios
      .post(`${serverClient}/api/v1/wardrobe`, unitFillData)
      .then((response) => {
        const newWardrobe = response.data.data;
        ...
        xhr.open(
          "PUT",
          `${serverClient}/api/v1/wardrobe/${newWardrobe._id}/upload`
        );
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>WARDROBE: UNIT</Text>
      </View>
      <View style={styles.view2}>
        <ScrollView style={{ flex: 1 }}>
          <FormText
            label="Enter unit's Wardrobe status: "
            placeholder="Please enter Wardrobe status"
            iconName="file-text"
            value={statusOf}
            onChangeText={(value) => setStatusOf(value)}
          />
          <FormText
            label="Enter unit's Wardrobe reason: "
            placeholder="Please enter Wardrobe reason"
            iconName="award"
            value={reason}
            onChangeText={(value) => setReason(value)}
          />
          <FormText
            label="Enter unit's Wardrobe fire rate: "
            placeholder="Please enter Wardrobe fire rate"
            iconName="star"
            value={fireRating}
