...
const ElectricalDetail = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${serverClient}/api/v1/electrical/${props.route.params.id}`)
      .then((response) => {
        setElectrical(response.data.data);
      })
  ...
  const sendEmail = () => {
    setIsLoading(true);
    axios
      .post(`${serverClient}/api/v1/electrical/${props.route.params.id}/send`, {
        email: email,
      })
      .then((response) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>All Electrical details:</Text>
      </View>
      <Animatable.View animation="bounceInUp" style={styles.view2}>
        ...
        <View style={styles.componentContent}>
          ...
          <Text style={styles.textValueStyle}> {electrical.statusOf}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Reason: </Text>
          <Text style={styles.textValueStyle}> {electrical.reason}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Fire rating: </Text>
          <Text style={styles.textValueStyle}> {electrical.fireRating}</Text>
        </View>
        <View
          style={{
            flexDirection: "row",
