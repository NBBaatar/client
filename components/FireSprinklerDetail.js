...
const FireSprinklerDetail = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${serverClient}/api/v1/firesprinkler/${props.route.params.id}`)
      .then((response) => {
        setFireSprinkler(response.data.data);
      })
  ...
  const sendEmail = () => {
    ...
    axios
      .post(
        `${serverClient}/api/v1/firesprinkler/${props.route.params.id}/send`,
        {
          email: email,
        }
      )
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>All Fire sprinkler details:</Text>
      </View>
      <Animatable.View animation="bounceInUp" style={styles.view2}>
        ...
        <View style={styles.componentContent}>
          ...
          <Text style={styles.textValueStyle}> {fireSprinkler.statusOf}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Reason: </Text>
          <Text style={styles.textValueStyle}> {fireSprinkler.reason}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Fire rating: </Text>
          <Text style={styles.textValueStyle}> {fireSprinkler.fireRating}</Text>
        </View>
        <View
          style={{
            flexDirection: "row",
        ...
        <Image
          key={fireSprinkler._id}
          source={{
            uri: fireSprinkler.photo?.[0].uri,
          }}
          style={styles.item}
        />
...
export default FireSprinklerDetail;
