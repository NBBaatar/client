...
const DefectsLand = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${serverClient}/api/v1/units/${props.route.params.id}/defects`)
      .then((response) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      ...
      <Animatable.View animation="bounceInUp" style={styles.view2}>
        ...
        <FormButton
          onPress={() => {
            props.navigation.navigate("Defect add", {
              id: id,
              building: building,
            });
          }}
        ...
        <FlatList
          nestedScrollEnabled
          data={defects}
          keyExtractor={(defect) => defect._id}
          renderItem={(el) => {
            return (
              <View>
                <TouchableOpacity
                  onPress={() => {
                    props.navigation.navigate("Defects detail", {
                      id: el.item._id,
                      unit: unit,
                    });
                  }}
