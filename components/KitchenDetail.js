...
const KitchenDetail = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${serverClient}/api/v1/kitchen/${props.route.params.id}`)
      .then((response) => {
  ...
  const sendEmail = () => {
    setIsLoading(true);
    axios
      .post(`${serverClient}/api/v1/kitchen/${props.route.params.id}/send`, {
        email: email,
      })
      .then((response) => {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      ...
      <Animatable.View animation="bounceInUp" style={styles.view2}>
        ...
        <View style={styles.componentContent}>
          ...
          <Text style={styles.textValueStyle}> {kitchen.statusOf}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Reason: </Text>
          <Text style={styles.textValueStyle}> {kitchen.reason}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Fire rating: </Text>
          <Text style={styles.textValueStyle}> {kitchen.fireRating}</Text>
        </View>
        <View
          style={{
            flexDirection: "row",
