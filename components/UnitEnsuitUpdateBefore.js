...
const UnitEnsuitUpdateBefore = (props) => {
  ...
  const [balconyLevelAfter, setBalconyLevelAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].ensuit?.[0].level,
  ]);
  const [balconyAngleAfter, setBalconyAngleAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].ensuit?.[0].angle,
  ]);
  const [balconyConcrateAfter, setBalconyConcrateAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].ensuit?.[0].concrate,
  ]);
  const [balconyPatchAfter, setBalconyPatchAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].ensuit?.[0].patch,
  ]);
  const [balconySubStraightAfter, setBalconySubStraightAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].ensuit?.[0].subStraight,
  ]);
  ...
  const handleUpdate = () => {
    axios
      ...
      .then((response) => {
        ...
        xhr.open(
          "PUT",
          `${serverClient}/api/v1/units/${props.route.params.id}/ensuit-before`
        );
        xhr.send(fmData);
