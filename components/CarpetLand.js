...
const CarpetLand = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${serverClient}/api/v1/units/${props.route.params.id}/carpet`)
      .then((response) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      ...
      <Animatable.View animation="bounceInUp" style={styles.view2}>
        ...
        <FormButton
          onPress={() => {
            props.navigation.navigate("Carpet add", {
              id: id,
              building: building,
            });
          }}
          ...
          renderItem={(el) => {
            return (
              <View>
                <TouchableOpacity
                  onPress={() => {
                    props.navigation.navigate("Carpet detail", {
                      id: el.item._id,
                      unit: unit,
                    });
                  }}
