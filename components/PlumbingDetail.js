...
const PlumbingDetail = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${serverClient}/api/v1/plumbing/${props.route.params.id}`)
      .then((response) => {
  ...
  const sendEmail = () => {
    setIsLoading(true);
    axios
      .post(`${serverClient}/api/v1/plumbing/${props.route.params.id}/send`, {
        email: email,
      })
      .then((response) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>All Plumbing details:</Text>
      </View>
      <Animatable.View animation="bounceInUp" style={styles.view2}>
        ...
        <View style={styles.componentContent}>
          ...
          <Text style={styles.textValueStyle}> {plumbing.statusOf}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Reason: </Text>
          <Text style={styles.textValueStyle}> {plumbing.reason}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Fire rating: </Text>
          <Text style={styles.textValueStyle}> {plumbing.fireRating}</Text>
        </View>
        <View
          style={{
            flexDirection: "row",
