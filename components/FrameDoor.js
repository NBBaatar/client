...
const FrameDoor = (props) => {
  const buildingId = props.route.params.unit.building;
  const [unitData, setUnitData] = useState([]);
  const [statusOf, setStatusOf] = useState([
    unitData?.units?.[0].framingDoor?.[0].statusOf,
  ]);
  const [reason, setReason] = useState([
    unitData?.units?.[0].framingDoor?.[0].reason,
  ]);
  const [fireBox, setFireBox] = useState([
    unitData?.units?.[0].framingDoor?.[0].fireBox,
  ]);
  const [fireRating, setFireRating] = useState([
    unitData?.units?.[0].framingDoor?.[0].fireRating,
  ]);
            ...
            after: [
              {
                ...
                landry: [
                  {
                    level:
                      unitData.units?.[0].waterProof?.[0].after?.[0].landry?.[0]
                        .level,
                    angle:
                      unitData.units?.[0].waterProof?.[0].after?.[0].landry?.[0]
                        .angle,
                ...
                ensuit: [
                  {
                    level:
                      unitData.units?.[0].waterProof?.[0].after?.[0].ensuit?.[0]
                        .level,
                    angle:
                      unitData.units?.[0].waterProof?.[0].after?.[0].ensuit?.[0]
                        .angle,
        ...
        plumbing: [
          {
            ...
            photo: [
              {
                fileName: unitData.units?.[0].plumbing?.[0].photo?.[0].fileName,
        ...
        tiles: [
          {
            ...
            bathroom: [
              {
                statusOf: unitData.units?.[0].tiles?.[0].bathroom?.[0].statusOf,
                ...
                photo: [
                  {
                    fileName:
                      unitData.units?.[0].tiles?.[0].bathroom?.[0].photo?.[0]
                        .fileName,
                    uri: unitData.units?.[0].tiles?.[0].bathroom?.[0].photo?.[0]
                      .uri,
                  },
                ],
              },
        ...
        wardrobe: [
          {
            ...
            photo: [
              {
                fileName: unitData.units?.[0].wardrobe?.[0].photo?.[0].fileName,
  ...
  const handleUpdate = () => {
    axios
      .put(
        `${serverClient}/api/v1/units/${props.route.params.id}`,
        unitFillData
      )
      .then((response) => {
        ...
        xhr.open(
          "PUT",
          `${serverClient}/api/v1/units/${props.route.params.id}/doorframe`
        );
        xhr.send(fmData);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>DOOR FRAMING: UNIT DETAILS</Text>
      </View>
      <View style={styles.view2}>
        <ScrollView style={{ flex: 1 }}>
          <FormText
            label="Enter unit's Door frame status: "
            placeholder="Please enter Door frame status"
            iconName="file-text"
            value={statusOf}
            onChangeText={(value) => setStatusOf(value)}
          />
          <FormText
            label="Enter unit's Door frame reason: "
            placeholder="Please enter Door frame reason"
            iconName="award"
            value={reason}
            onChangeText={(value) => setReason(value)}
          />
          <FormText
            label="Enter unit's Door frame fire rate: "
            placeholder="Please enter Door frame fire rate"
            iconName="star"
            value={fireRating}
          ...
          <FormRadioButton
            ...
            label="Is fire box installed?"
            value={fireBox}
            onValueChange={(checked) => setFireBox((checked) => !checked)}
          />
