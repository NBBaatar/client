...
const WindowsLand = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${serverClient}/api/v1/units/${props.route.params.id}/windows`)
      .then((response) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>All Windows details:</Text>
      </View>
      <Animatable.View animation="bounceInUp" style={styles.view2}>
        ...
        <FormButton
          onPress={() => {
            props.navigation.navigate("Windows add", {
              id: id,
              building: building,
            });
          }}
          ...
          renderItem={(el) => {
            return (
              <View>
                <TouchableOpacity
                  onPress={() => {
                    props.navigation.navigate("Windows detail", {
                      id: el.item._id,
                      unit: unit,
                    });
                  }}
