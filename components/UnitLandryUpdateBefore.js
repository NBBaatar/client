...
const UnitLandryUpdateBefore = (props) => {
  ...
  const [balconyLevelAfter, setBalconyLevelAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].landry?.[0].level,
  ]);
  const [balconyAngleAfter, setBalconyAngleAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].landry?.[0].angle,
  ]);
  const [balconyConcrateAfter, setBalconyConcrateAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].landry?.[0].concrate,
  ]);
  const [balconyPatchAfter, setBalconyPatchAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].landry?.[0].patch,
  ]);
  const [balconySubStraightAfter, setBalconySubStraightAfter] = useState([
    unitData?.units?.[0].waterProof?.[0].before?.[0].landry?.[0].subStraight,
  ]);
  ...
  const handleUpdate = () => {
    axios
      ...
      .then((response) => {
        ...
        xhr.open(
          "PUT",
          `${serverClient}/api/v1/units/${props.route.params.id}/landry-before`
        );
        xhr.send(fmData);
