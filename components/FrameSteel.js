...
const FrameSteelDetail = (props) => {
  const buildingId = props.route.params.unit.building;
  const [unitData, setUnitData] = useState([]);
  const [statusOf, setStatusOf] = useState([
    unitData?.units?.[0].framingSteel?.[0].statusOf,
  ]);
  const [reason, setReason] = useState([
    unitData?.units?.[0].framingSteel?.[0].reason,
  ]);

  const [fireRating, setFireRating] = useState([
    unitData?.units?.[0].framingSteel?.[0].fireRating,
  ]);
  ...
  const handleUpdate = () => {
    axios
      ...
      .then((response) => {
        ...
        xhr.open(
          "PUT",
          `${serverClient}/api/v1/units/${props.route.params.id}/steelframe`
        );
        xhr.send(fmData);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>STEEL FRAME: UNIT</Text>
      </View>
      <View style={styles.view2}>
        <ScrollView style={{ flex: 1 }}>
          <FormText
            label="Enter unit's Steel frame status: "
            placeholder="Please enter Steel frame status"
            iconName="file-text"
            value={statusOf}
            onChangeText={(value) => setStatusOf(value)}
          />
          <FormText
            label="Enter unit's Steel frame reason: "
            placeholder="Please enter Steel frame reason"
            iconName="award"
            value={reason}
            onChangeText={(value) => setReason(value)}
          />
          <FormText
            label="Enter unit's Steel frame fire rate: "
            placeholder="Please enter Steel frame fire rate"
            iconName="star"
            value={fireRating}
...
export default FrameSteelDetail;
