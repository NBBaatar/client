...
const DryFireCableDetail = (props) => {
  const [dryFireCable, setDryFireCable] = useState([]);
  ...
  useEffect(() => {
    axios
      .get(`${serverClient}/api/v1/dryfirecable/${props.route.params.id}`)
      .then((response) => {
        setDryFireCable(response.data.data);
      })
  ...
  const sendEmail = () => {
    ...
    axios
      .post(
        `${serverClient}/api/v1/dryfirecable/${props.route.params.id}/send`,
        {
          email: email,
        }
      )
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#36393e" }}>
      <View style={styles.view1}>
        <Text style={styles.componentHeader}>All Dry fire cable details:</Text>
      </View>
      <Animatable.View animation="bounceInUp" style={styles.view2}>
        ...
        <View style={styles.componentContent}>
          ...
          <Text style={styles.textValueStyle}> {dryFireCable.statusOf}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Reason: </Text>
          <Text style={styles.textValueStyle}> {dryFireCable.reason}</Text>
        </View>
        <View style={styles.componentContent}>
          <Text style={styles.textLabelStyle}>Fire rating: </Text>
          <Text style={styles.textValueStyle}> {dryFireCable.fireRating}</Text>
        </View>
        <View
          style={{
            flexDirection: "row",
        ...
        <Image
          key={dryFireCable._id}
          source={{
            uri: dryFireCable.photo?.[0].uri,
          }}
          style={styles.item}
        />
...
export default DryFireCableDetail;
